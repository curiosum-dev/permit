defmodule PhoenixAuthorization.Plug do
  @moduledoc """
  ...
  action, and delegates to PhoenixAuthorization to determine whether the action is authorized based
  on current user's role.

  Current user is always fetched from `conn.assigns[:current_user]`, and their role is taken
  from the module specified in the app's `use PhoenixAuthorization` directive. For instance if it's
  ...
  alias PhoenixAuthorization.{Resolver, Types}
